...
###### Encoder
![encoder network][image 4]

The encoder portion is a convolution network that reduces to a deeper 1x1 convolution layer. This is to identify the important **features** in our image, keep those features in memory, remove the noise of other pixels, and decrease the width and height, while increasing layer depth.
**Separable Convolutions** is a technique that reduces the number of parameters needed, thus increasing efficiency for the encoder network.

Our function from lecture to determine the height/width  of our subsequent layers: `(N-F + 2*P)/S + 1`
We can then calcualte what the size of our first layer will be: **encoder_height_width = (160-3 + 2*1)/2 +1** , so that **encoder_dimension = 80x80x64** since we chose 64 filters.
...
![1x1 convolution][image 5]

As we can know, in **mage Classification**, the Convolution Neural Network (CNN) will end with a **fully connected**. That will output a vector of **n classes**, each class which assigned a probability between `0-1` where the entire vector sums to `1`, all dimensions are flattened into a single vector losing the original spacial structure of the input.
But because we need to answer the question of **WHERE**, not just **WHAT**, we'll use a **1x1 convolutional layer** for the task of **semantic segmentation**. This will allow us to assign individual pixels to one of the three classes.
 **1x1 convolutional layers** provides us the following advantages:
- It helps our network more flexible by allowing different sized input images.
- It decreases dimensions, while **preserve spatial information** of the image, which allows us to output a segmented image
- It adds depth to our model.

And next, we've used a **1x1 convolutional** layer in the middle of the network, it allows use to take in any sized image, as opposed to a fully connected layer that would require a specific set of input dimensions.
...
![decoder network][image 6]

Finally to the next section of the model. The decoder block upsamples the output from the 1x1 convolution back to the original input format, through the use of a series of transpose convolution layers.
Upsampling is very important in the second half of our FCN in order to transform the important features we learned from encoding into the final segmented image in the output. There are, in fact, several ways to achieve upsampling. In this section, we will cover another method called **bilinear upsampling or bilinear interpolation**.
**Bilinear Upsampling** helps transform our downsampled image back into the resolution of our original input image.
We'll be repeatedly using **Bilinear Upsampling** in the Decoder Layer. In addition to **bilinear_upsample**, we'll use concatenation with **layers.concatenate**, which will help us implement skip connections, finally, we add **separable_conv2d_batchnorm** function.
```
...
In addition to **bilinear_upsample**, we'll use concatenation with **layers.concatenate**, which will help us implement **skip connections**:

![skipconnect][image 7]

Allow the network to use information from multiple resolution scales resulting on more precise segmentation decisions, as well as the **separable_conv2d_batchnorm** function. These functions will make up our `decoder_block` function:
```
